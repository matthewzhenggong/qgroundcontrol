...
QGC_LOGGING_CATEGORY(ZTParameterMetaDataLog, "ZTParameterMetaDataLog")

ZTParameterMetaData::ZTParameterMetaData(void)
...
QVariant ZTParameterMetaData::_stringToTypedVariant(const QString& string, FactMetaData::ValueType_t type, bool* convertOk)
{
    QVariant var(string);
...
void ZTParameterMetaData::loadParameterFactMetaDataFile(const QString& metaDataFile)
{
    qCDebug(ParameterManagerLog) << "ZTParameterMetaData::loadParameterFactMetaDataFile" << metaDataFile;
    ...
    qCDebug(ZTParameterMetaDataLog) << "Loading parameter meta data:" << metaDataFile;
    ...
    while (!xml.atEnd()) {
        if (xml.isStartElement()) {
            ...
            if (elementName == "parameters") {
                ...
                qCDebug(ZTParameterMetaDataLog) << "Found group: " << factGroup;
                ...
                qCDebug(ZTParameterMetaDataLog) << "Found parameter name:" << name << " type:" << type << " default:" << strDefault;
                ...
                if (_mapParameterName2FactMetaData.contains(name)) {
                    ...
                    qCWarning(ZTParameterMetaDataLog) << "Duplicate parameter found:" << name;
                    badMetaData = true;
                    ...
                    if (xml.attributes().hasAttribute("default") && !strDefault.isEmpty()) {
                        ...
                        if (metaData->convertAndValidateRaw(strDefault, false, varDefault, errorString)) {
                            ...
                            qCWarning(ZTParameterMetaDataLog) << "Invalid default value, name:" << name << " type:" << type << " default:" << strDefault << " error:" << errorString;
                        }
                    }
                }
                ...
                if (!badMetaData) {
                    if (metaData) {
                        if (elementName == "short_desc") {
                            ...
                            text = text.replace("\n", " ");
                            qCDebug(ZTParameterMetaDataLog) << "Short description:" << text;
                            metaData->setShortDescription(text);

                        } else if (elementName == "long_desc") {
                            QString text = xml.readElementText();
                            text = text.replace("\n", " ");
                            qCDebug(ZTParameterMetaDataLog) << "Long description:" << text;
                            metaData->setLongDescription(text);

                        } else if (elementName == "min") {
                            QString text = xml.readElementText();
                            qCDebug(ZTParameterMetaDataLog) << "Min:" << text;

                            QVariant varMin;
                            if (metaData->convertAndValidateRaw(text, true /* convertOnly */, varMin, errorString)) {
                                metaData->setRawMin(varMin);
                            } else {
                                qCWarning(ZTParameterMetaDataLog) << "Invalid min value, name:" << metaData->name() << " type:" << metaData->type() << " min:" << text << " error:" << errorString;
                            }

                        } else if (elementName == "max") {
                            QString text = xml.readElementText();
                            qCDebug(ZTParameterMetaDataLog) << "Max:" << text;

                            QVariant varMax;
                            if (metaData->convertAndValidateRaw(text, true /* convertOnly */, varMax, errorString)) {
                                metaData->setRawMax(varMax);
                            } else {
                                qCWarning(ZTParameterMetaDataLog) << "Invalid max value, name:" << metaData->name() << " type:" << metaData->type() << " max:" << text << " error:" << errorString;
                            }

                        } else if (elementName == "unit") {
                            QString text = xml.readElementText();
                            qCDebug(ZTParameterMetaDataLog) << "Unit:" << text;
                            metaData->setRawUnits(text);

                        } else if (elementName == "decimal") {
                            QString text = xml.readElementText();
                            qCDebug(ZTParameterMetaDataLog) << "Decimal:" << text;
                            ...
                            if (convertOk) {
                                ...
                                qCWarning(ZTParameterMetaDataLog) << "Invalid decimals value, name:" << metaData->name() << " type:" << metaData->type() << " decimals:" << text << " error: invalid number";
                            }

                        } else if (elementName == "reboot_required") {
                            QString text = xml.readElementText();
                            qCDebug(ZTParameterMetaDataLog) << "RebootRequired:" << text;
                            ...
                            qCDebug(ZTParameterMetaDataLog) << "parameter value:"
                            ...
                            if (metaData->convertAndValidateRaw(enumValueStr, false /* validate */, enumValue, errorString)) {
                                ...
                                qCDebug(ZTParameterMetaDataLog) << "Invalid enum value, name:" << metaData->name()
                            ...
                            if (ok) {
                                ...
                                qCWarning(ZTParameterMetaDataLog) << "Invalid value for increment, name:" << metaData->name() << " increment:" << text;
                            }
                            ...
                            if (ok) {
                                ...
                                if (bit < 31) {
                                    ...
                                    if (metaData->convertAndValidateRaw(bitmaskRawValue, true, bitmaskValue, errorString)) {
                                        ...
                                        qCDebug(ZTParameterMetaDataLog) << "Invalid bitmask value, name:" << metaData->name()
                                                                         << " type:" << metaData->type() << " value:" << bitmaskValue
                                                                         << " error:" << errorString;
                                    }
                                } else {
                                    qCWarning(ZTParameterMetaDataLog) << "Invalid value for bitmask, bit:" << bit;
                                }
                            }
                        } else {
                            qCDebug(ZTParameterMetaDataLog) << "Unknown element in XML: " << elementName;
                        }
                    } else {
            ...
            if (elementName == "parameter") {
                ...
                if (metaData->defaultValueAvailable()) {
                    ...
                    if (!metaData->convertAndValidateRaw(metaData->rawDefaultValue(), false /* convertOnly */, var, errorString)) {
                        qCWarning(ZTParameterMetaDataLog) << "Invalid default value, name:" << metaData->name() << " type:" << metaData->type() << " default:" << metaData->rawDefaultValue() << " error:" << errorString;
                    }
                }
...
void ZTParameterMetaData::addMetaDataToFact(Fact* fact, MAV_TYPE vehicleType)
{
...
void ZTParameterMetaData::getParameterMetaDataVersionInfo(const QString& metaDataFile, int& majorVersion, int& minorVersion)
{
    QFile xmlFile(metaDataFile);
